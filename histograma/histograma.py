#Import urllib for downloading and uncompress the file
...
rated_movies[rated_movies['genres'] == 'Shawshank Redemption, The (1994)']


# rated_movies = rated_movies.pop(0)
# You can aggregate data like this
grouped = rated_movies.groupby('genres')
...
# Or the short way
grouped = rated_movies.groupby('genres').sum()
...
tcinco= top5_dict['rating'].values()
name = top5_dict['rating'].keys()
keys = list(name)
datos5= list(tcinco)


plt.xticks( range(1,6), keys,rotation = "vertical", fontsize = 9, ha= 'left', color="black")
hist(datos5, color='pink',bins=5)
plt.xlabel('Generos', color="orange")
plt.ylabel('Rating', color="orange")
plt.title('Histograma TOP-5', color="red")
plt.tight_layout()
print(name)
print(tcinco)
...
tveinte= top20_dict['rating'].values()
name2 = top20_dict['rating'].keys()
keys2 = list(name2)
datos20= list(tveinte)
print(name2)
print(tveinte)
plt.xticks(range(1, 21), keys2, rotation = "vertical", fontsize = 6, ha= 'left', color="black")
hist(datos20, color='pink')
plt.xlabel('Generos', color="orange")
plt.ylabel('Rating', color="orange")
plt.title('Grafica TOP-20', color="red")
plt.tight_layout()
plt.savefig("hist20.png")
...
for name, value in top5_items:
    frames.append(rated_movies[rated_movies['genres'] == name])
...
ratings_by_title = rated_movies.groupby('genres').size()
